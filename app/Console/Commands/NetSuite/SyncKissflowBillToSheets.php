...
class SyncKissflowBillToSheets extends Command
{
    protected $signature = 'kissflow:sync-bill-to-sheets {--sheet=KF ID : Sheet name to read Kissflow IDs from}';
    ...
    {
        $this->info('Starting Kissflow Bill data sync to Google Sheets...');
        ...
        try {
            ...
            $rows = $billSheetsService->readSheet($sourceSheetName);
                ...
                if (!empty($existingKissflowId)) {
                    ...
                    try {
                        ...
                        if ($searchResult && !empty($searchResult['id'])) {
                            ...
                            $billSheetsService->updateCell($sourceSheetName, $cellAddress, $kissflowId);
            ...
            if (!empty($prRows)) {
                ...
                $prSheetRows = $billSheetsService->readSheet('PR');
                ...
                if (empty($prHeaders)) {
                    ...
                    $billSheetsService->appendToSheet('PR', [$expectedPRHeaders]);
                ...
                $billSheetsService->appendToSheet('PR', $prDataToWrite);
            ...
            if (!empty($lineItemRows)) {
                ...
                $lineItemSheetRows = $billSheetsService->readSheet('Line Item');
                ...
                if (empty($lineItemHeaders)) {
                    ...
                    $billSheetsService->appendToSheet('Line Item', [$expectedLineItemHeaders]);
                ...
                $billSheetsService->appendToSheet('Line Item', $lineItemDataToWrite);
            ...
            $this->error('Error syncing Kissflow Bill data: ' . $e->getMessage());
