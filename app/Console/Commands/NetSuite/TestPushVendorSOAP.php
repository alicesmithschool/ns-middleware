...
class TestPushVendorSOAP extends Command
{
    protected $signature = 'netsuite:test-push-vendor-soap {--malaysian : Create Malaysian vendor} {--non-malaysian : Create non-Malaysian vendor}';
    protected $description = 'Test vendor creation via SOAP API with dummy data';

    public function handle(NetSuiteService $netSuiteService)
    {
        $isMalaysian = $this->option('malaysian') || !$this->option('non-malaysian');
        
        $this->info('Testing vendor creation via SOAP with dummy data...');
        ...
        if (!$this->confirm('Do you want to create this vendor in NetSuite via SOAP?')) {
            $this->info('Cancelled.');
            return Command::SUCCESS;
        }

        // Create vendor via SOAP
        try {
            $this->info('Creating vendor via SOAP API...');
            $result = $netSuiteService->createVendorWithTINFields($vendorData);

            if ($result['success']) {
                $this->info('✓ Vendor created successfully via SOAP!');
                $this->line("Internal ID: {$result['internal_id']}");
                $this->line("Entity ID: {$result['entity_id']}");
            } else {
                $this->error('✗ Failed to create vendor via SOAP');
    ...
    {
        return [
            'company_name' => 'Test SOAP Malaysian Vendor Sdn Bhd',
            // 'entity_id' => 'TEST_SOAP_MY_' . time(), // Let NetSuite auto-generate
            'email' => 'test-soap@malaysian-vendor.com',
            'phone' => '+60123456789',
            'is_inactive' => false,
            
            // Address
            'address_1' => '123 Jalan Test SOAP',
            'address_2' => 'Unit 5-01',
            'city' => 'Kuala Lumpur',
            'state' => 'Selangor',
            'zip' => '50000',
            ...
            // TIN/E-Invoicing fields with correct names
            'tin_no' => 'C12345678901234',
            'tin_registered_name' => 'Test SOAP Malaysian Vendor Sdn Bhd',
            'tin_sst_register_no' => 'A01-2345-67890123',
            'tin_msic_id' => '1', // 00000 : NOT APPLICABLE
            'tin_address_line1' => '123 Jalan Test SOAP',
    ...
    {
        return [
            'company_name' => 'Test SOAP US Vendor Inc',
            'entity_id' => 'TEST_SOAP_US_' . time(),
            'email' => 'test-soap@us-vendor.com',
            'phone' => '+15551234567',
            'is_inactive' => false,
            ...
            'tin_no' => 'EI00000000030',
            'tin_registered_name' => 'Test SOAP US Vendor Inc',
